...
An LLM can confidently assign the wrong category/priority or cite a document that does not support the issue. I mitigate this by separating retrieval from generation, passing only retrieved KB context to the model, using a controlled category/urgency vocabulary, validating the response with Pydantic, and rejecting KB citations that were not actually retrieved. The evaluation harness also checks core classification fields and KB relevance. A content-addressed cache stores the validated structured result for the exact model/prompt/schema/input combination, so repeated evaluation of the same case returns the same result. In production I would monitor disagreement rates, low-confidence cases, and human overrides.
...
The supplied dataset is synthetic, but the architecture assumes real support tickets may contain PII. The application does not scrape external sources or enrich records with outside customer information. Only the minimum ticket/account context required for the task is sent to the LLM adapter. API credentials are read from environment variables and excluded through `.gitignore`; `.env.example` contains placeholders only. For a real deployment, I would add PII detection/redaction where policy requires it, use an approved enterprise LLM endpoint with appropriate retention controls, encrypt traffic, restrict logs, and ensure prompts/responses containing customer data are not written to ordinary application logs.
